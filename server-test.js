var uploader = require('./dist/node.js'); // This file is just a test to make sure node-style imports are configured properly
// I just ran this with `node server-test.js` to verify the output.
// It may be useful to someone in the future.
// eslint-disable-next-line no-console
console.log(uploader);

const json = `{
  "entityType": "group",
  "performanceYear": 2021,
  "programName": "app1",
  "taxpayerIdentificationNumber": "000101010",
  "measurementSets": [{
      "submissionMethod": "registry",
      "category": "quality",
      "performanceStart": "2021-01-01",
      "performanceEnd": "2021-12-31",
      "programName": "app1",
      "measurements": [
        {
          "measureId": "001",
          "value": {
            "isEndToEndReported": false,
            "performanceMet": 180,
            "eligiblePopulation": 200,
            "eligiblePopulationExclusion": 0,
            "eligiblePopulationException": 0,
            "performanceNotMet": 20,
            "performanceRate": 100
          }
        },
        
        {
          "measureId": "236",
          "value": {
            "isEndToEndReported": false,
            "performanceMet": 180,
            "eligiblePopulation": 200,
            "eligiblePopulationExclusion": 0,
            "eligiblePopulationException": 0,
            "performanceNotMet": 20,
            "performanceRate": 100
          }
        }
      ]
    },
    {
      "submissionMethod": "electronicHealthRecord",
      "category": "ia",
      "performanceStart": "2021-01-01",
      "performanceEnd": "2021-12-31",
      "programName": "app1",
      "measurements": [{
          "measureId": "IA_AHE_1",
          "value": true
        },
        {
          "measureId": "IA_AHE_5",
          "value": true
        },
        {
          "measureId": "IA_AHE_7",
          "value": true
        }
      ]
    },
    {
      "submissionMethod": "electronicHealthRecord",
      "category": "pi",
      "performanceStart": "2021-01-01",
      "performanceEnd": "2021-12-31",
      "measureSet": null,
      "cehrtId": "XX15EXXXXXXXXXX",
      "programName": "app1",
      "measurements": [{
          "measureId": "PI_EP_1",
          "value": {
            "numerator": 100.0,
            "denominator": 200.0
          }
        },
        {
          "measureId": "PI_EP_2",
          "value": true
        },
        {
          "measureId": "PI_HIE_1",
          "value": {
            "numerator": 100.0,
            "denominator": 200.0
          }
        },
        {
          "measureId": "PI_HIE_4",
          "value": {
            "numerator": 100.0,
            "denominator": 200.0
          }
        },
        {
          "measureId": "PI_INFBLO_1",
          "value": true
        },
        {
          "measureId": "PI_ONCACB_1",
          "value": true
        },
        {
          "measureId": "PI_ONCDIR_1",
          "value": true
        },
        {
          "measureId": "PI_PEA_1",
          "value": {
            "numerator": 100.0,
            "denominator": 200.0
          }
        },
        {
          "measureId": "PI_PHCDRR_1",
          "value": true
        },
        {
          "measureId": "PI_PHCDRR_2",
          "value": true
        },
        {
          "measureId": "PI_PPHI_1",
          "value": true
        }
      ]
    }
  ]
}`;

//manually call validate with json file here.
uploader.fileUploader(json, 'JSON', undefined, 'http://local.semanticbits.com:8081', (err, createdMeasurementSets) => {
  if(err) {
    console.log(err);
  }

  console.log(createdMeasurementSets);
});
