...
var _fileUploader = __webpack_require__(16);
...
var _fileUploaderUtil = __webpack_require__(17);
 ...
 * @return {Promise}
 */
function fileUploader(submissionBody, submissionFormat, JWT, baseSubmissionURL, callback) {
  var validatedSubmission = void 0;
  var existingSubmission = void 0;
  var errs = [];
  var createdMeasurementSets = [];
  ...
  return _fileUploaderUtil.fileUploaderUtil.validateSubmission(submissionBody, submissionFormat, baseOptions).then(function (validSubmission) {
    validatedSubmission = validSubmission;
    return _fileUploaderUtil.fileUploaderUtil.getExistingSubmission(validatedSubmission, baseOptions);
  }).then(function (existingSubmissionReturned) {
    existingSubmission = existingSubmissionReturned;
    var firstMeasurementSetPromise = void 0;
    ...
    if (!existingSubmission) {
      var firstMeasurementSet = Object.assign({}, validatedSubmission.measurementSets.pop(), { submission: {
      ...
      firstMeasurementSetPromise = _fileUploaderUtil.fileUploaderUtil.postMeasurementSet(firstMeasurementSet, baseOptions);
    };

    return firstMeasurementSetPromise;
  }).then(function (firstMeasurementSetOutput) {
    ...
    var postAndPutPromises = _fileUploaderUtil.fileUploaderUtil.submitMeasurementSets(existingSubmission, validatedSubmission, baseOptions);

    // Transform rejected promises into Errors so they are caught and don't short-circuit
    // the others
    var caughtPromises = postAndPutPromises.map(function (promise) {
      return promise.catch(Error);
    });
    return Promise.all(caughtPromises);
  }).then(function (postAndPutOutputs) {
    // Aggregate the errors and created measurementSets
    postAndPutOutputs.forEach(function (postOrPutOutput) {
...
function validateSubmission(submission, submissionFormat, baseOptions) {
  var headers = Object.assign({}, baseOptions.headers);
  ...
  var validateSubmissionOptions = Object.assign({}, baseOptions, {
    headers: headers,
    data: submission
  });

  return _axios.axios.post(baseOptions.url + '/submissions/validate', validateSubmissionOptions).then(function (body) {
    var validatedSubmission = JSON.parse(body).data.submission;
...
function getExistingSubmission(submission, baseOptions) {
  ...
  var queryParamString = Object.keys(queryParams).map(function (key) {
    return encodeURIComponent(key) + '=' + encodeURIComponent(queryParams[key]);
  }).join('&');

  var getSubmissionsOptions = Object.assign({}, baseOptions, {
    headers: Object.assign({}, baseOptions.headers, {
      'qpp-taxpayer-identification-number': submission.taxpayerIdentificationNumber
    })
  });

  return _axios.axios.get(baseOptions.url + '/submissions?' + queryParamString, getSubmissionsOptions).then(function (body) {
    var jsonBody = JSON.parse(body);
    var existingSubmissions = jsonBody.data.submissions;
    ...
    var matchingExistingSubmissions = jsonBody.data.submissions.filter(function (existingSubmission) {
...
function putMeasurementSet(measurementSet, baseOptions, measurementSetId) {
  var putMeasurementSetOptions = Object.assign({}, baseOptions, {
    data: JSON.stringify(measurementSet)
  });

  return _axios.axios.put(baseOptions.url + '/measurement-sets/' + measurementSetId, putMeasurementSetOptions).then(function (body) {
...
function postMeasurementSet(measurementSet, baseOptions) {
  var postMeasurementSetOptions = Object.assign({}, baseOptions, {
    data: JSON.stringify(measurementSet)
  });

  return _axios.axios.post(baseOptions.url + '/measurement-sets', postMeasurementSetOptions).then(function (body) {
...
function submitMeasurementSets(existingSubmission, submission, baseOptions) {
  var promises = [];
  submission.measurementSets.forEach(function (measurementSet) {
    var measurementSetToSubmit = void 0;
    var existingMeasurementSets = [];
    ...
    var matchingMeasurementSets = existingMeasurementSets.filter(function (existingMeasurementSet) {
      return existingMeasurementSet.submissionMethod === measurementSet.submissionMethod && existingMeasurementSet.category === measurementSet.category;
    });
    if (matchingMeasurementSets.length > 0) {
      // Do a PUT
      var matchingMeasurementSetId = matchingMeasurementSets[0].id;
...
module.exports = {
  ...
  getExistingSubmission: getExistingSubmission,
  putMeasurementSet: putMeasurementSet,
  postMeasurementSet: postMeasurementSet,
  submitMeasurementSets: submitMeasurementSets
};
